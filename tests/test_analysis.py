"""Тесты анализа спектра на синтетических лоренцевых провалах (без lumapi)."""
...
def test_single_resonance_q_recovered():
    ...
    assert abs(q - 3100.0) / 3100.0 < 0.1   # в пределах 10%
    assert depth > 0.5
...
def test_unsorted_input_handled():
    ...
    # перемешиваем порядок — analyze_spectrum должен отсортировать сам
